...
//! the application's natural event flow, using timers only as a fallback.
...
/// }
///
/// // After render - schedule fallback
/// ticker.schedule_fallback();
///
/// // In event loop - poll for fallback timer
/// if ticker.poll_fallback(&mut cx).is_ready() {
///     needs_render = true;
/// }
/// ```
pub struct OpportunisticDebouncedTicker<T: Tickable> {
    ...
    /// Channel for scheduling fallback ticks
    fallback_tx: Sender<()>,
    /// Receiver for draining cancelled fallbacks
    fallback_rx: Receiver<()>,
    /// Debounced stream for fallback timing
    fallback_stream: Pin<Box<Debounced<()>>>,
    /// Performance metrics
...
impl<T: Tickable> OpportunisticDebouncedTicker<T> {
    ...
    pub fn with_early_window(target: T, early_window_percent: u8) -> Self {
        ...
        let (fallback_tx, fallback_rx) = bounded(1);

        // Create debounced stream with frame interval
        let fallback_stream = Box::pin(debounced(fallback_rx.clone(), frame_interval));
    ...
    pub fn on_any_event(&mut self) -> bool {
        ...
        if self.should_tick(now) {
            ...
            // Drain any pending fallback events to prevent double-tick
            while self.fallback_rx.try_recv().is_ok() {
                self.metrics.cancelled_fallbacks += 1;
            }
    ...
    /// Schedule a fallback tick via the debounced stream
    #[inline]
    pub fn schedule_fallback(&mut self) {
        if self.target.is_active() {
            // Send to our private channel - debounced stream handles timing
            let _ = self.fallback_tx.try_send(());
        } else {
            self.metrics.skipped_inactive += 1;
        }
    }

    /// Poll the fallback stream - returns true if a tick occurred
    pub fn poll_fallback(&mut self, cx: &mut Context<'_>) -> Poll<bool> {
        ...
        match self.fallback_stream.poll_next_unpin(cx) {
            Poll::Ready(Some(())) => {
                // Fallback timer fired - tick now
                self.target.tick();
    ...
    pub fn try_poll_fallback(&mut self) -> bool {
        ...
        matches!(self.poll_fallback(&mut cx), Poll::Ready(true))
    }

...
pub struct TickerMetrics {
    ...
    /// Ticks triggered by the fallback timer
    pub fallback_ticks: u64,
    /// Ticks that happened earlier than ideal due to sliding window
    pub early_ticks: u64,
    /// Fallback events that were cancelled (drained) after opportunistic ticks
    pub cancelled_fallbacks: u64,
...
impl TickerMetrics {
    ...
    pub fn opportunistic_ratio(&self) -> f32 {
        let total = self.opportunistic_ticks + self.fallback_ticks;
        if total == 0 {
            0.0
        } else {
    ...
    pub const fn total_ticks(&self) -> u64 {
        self.opportunistic_ticks + self.fallback_ticks
    }
}

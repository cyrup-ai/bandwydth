...
use crate::{
    display::{BandwidthUnitFamily, DisplayMode},
    network::{BandwidthClass, Connection, LocalSocket, Utilization},
    os::ProcessInfo,
};
...
/// Connection-specific bandwidth and metadata.
#[derive(Clone, Default)]
pub struct ConnectionData {
    /// Total bytes downloaded on this connection.
    pub total_bytes_downloaded: u128,
    /// Total bytes uploaded on this connection.
    pub total_bytes_uploaded: u128,
    /// Name of the process owning this connection.
...
pub struct UIState {
    ...
    pub total_bytes_uploaded: u128,
    /// Whether to show cumulative data vs current rate.
    pub cumulative_mode: bool,
    /// Whether to show DNS queries.
    pub show_dns: bool,
    ...
    /// Recent utilization data for averaging.
    pub utilization_data: VecDeque<UtilizationData>,
    /// Map of processes to their network data.
    pub processes_map: HashMap<ProcessInfo, NetworkData>,
    /// Map of remote addresses to their network data.
    pub remote_addresses_map: HashMap<IpAddr, NetworkData>,
    /// Map of connections to their data.
    ...
    /// Current display mode for tab navigation.
    pub display_mode: DisplayMode,
}

impl UIState {
    /// Update bandwidth classification and speed.
    pub fn update_bandwidth_stats(&mut self, stats: &crate::network::BandwidthStats) {
        self.bandwidth_class = stats.bandwidth_class.clone();
        self.current_speed = stats.current_speed;
    }

    /// Update the UI state with new network utilization data.
    pub fn update(
        &mut self,
        connections_to_procs: HashMap<LocalSocket, ProcessInfo>,
        network_utilization: Utilization,

...
impl Table {
    /// Create a table showing network connections and their bandwidth usage.
    ...
    /// Create a table showing processes and their bandwidth usage.
    ...
    /// Create a table showing remote addresses and their bandwidth usage.
    pub fn create_remote_addresses_table(
        ...
        let rows: Vec<_> = state
            ...
            .map(|(remote_address, data_for_remote_address)| {
                let hostname = ip_to_host.get(remote_address).cloned();
                let remote_address = display_ip_or_host(remote_address, &hostname);
                [
                    remote_address,
    ...
    pub fn render(&self, frame: &mut Frame, rect: Rect) {
        let (computed_layout, spacer_width) = {
            ...
            let &(_, layout) = self
                ...
                .unwrap_or_else(|| {
                    // Fallback to first layout if somehow no match found
                    self.width_cutoffs
                        .first()
                        .unwrap_or(&(0, DisplayLayout::C2([50, 50])))
                });

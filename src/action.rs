...
impl SpinnerId {
    /// Create a new spinner ID from a raw u8 value
    #[inline(always)]
...
impl SpinnerSpeed {
    /// Create a normal speed multiplier (1.0x)
    #[inline(always)]
    pub const fn normal() -> Self {
        Self(1.0)
    }

    /// Create a double speed multiplier (2.0x)
    #[inline(always)]
    pub const fn double() -> Self {
        Self(2.0)
    }

    /// Create a half speed multiplier (0.5x)
    #[inline(always)]
...
pub enum Action {
    ...
    SpinnerCreate(SpinnerId, crate::spinners::SpinnerPreset),
    ...
    /// Bandwidth monitoring thread signals UI to update
    UpdateUI,

    /// DNS resolution results ready
    DnsUpdate(std::collections::HashMap<std::net::IpAddr, String>),

    /// Bandwidth stats component requests UI update
    StatsUpdate,
}
